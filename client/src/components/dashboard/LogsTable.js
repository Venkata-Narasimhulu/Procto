...
//   { id: 'student_college_id', numeric: false, disablePadding: false, label: 'College ID' },
...
const headCells = [
  ...
  { id: 'student_name', numeric: false, disablePadding: false, label: 'Name' },
  { id: 'student_email', numeric: false, disablePadding: false, label: 'Email' },
  { id: 'collegeId', numeric: false, disablePadding: false, label: 'College ID' },
  { id: 'tab_change_count', numeric: true, disablePadding: false, label: 'Tab Changes' },
  { id: 'key_press_count', numeric: true, disablePadding: false, label: 'Prohibited Key Press' },
  { id: 'face_not_visible', numeric: false, disablePadding: false, label: 'Face Not Visible' },
  { id: 'multiple_faces_found', numeric: false, disablePadding: false, label: 'Multiple Faces Detected' },
...
export default function LogsTable(props) {
  ...
  const getData = async () => {
    try {
      await axios.get(`/api/exams/examsByProf?exam_code=${exam_code}&prof_email=${props.prof_email}`);
    } catch (err) {
    ...
    try {
      const response = await axios.post('/api/logs/allData', { exam_code });
      setErrorText('');
      setVisibility(true);

      const curr_logs = response.data.map((log, index) => ({
        s_no: index + 1,
        student_name: log.student_name || '',
        student_email: log.student_email || '',
        collegeId: log.collegeId || 'N/A',
        tab_change_count: log.tab_change_count || 0,
        key_press_count: log.key_press_count || 0,
        face_not_visible: log.face_not_visible ? 'Yes' : 'No',
        multiple_faces_found: log.multiple_faces_found ? 'Yes' : 'No',
        mobile_found: log.mobile_found ? 'Yes' : 'No',
        prohibited_object_found: log.prohibited_object_found ? 'Yes' : 'No',
      }));
                ...
                {stableSort(rows, getComparator(order, orderBy))
                  ...
                  .map((row) => (
                    <TableRow hover key={row.s_no}>
                      ...
                      <TableCell>{row.collegeId}</TableCell>
                      <TableCell align="right">{row.tab_change_count}</TableCell>
                      <TableCell align="right">{row.key_press_count}</TableCell>
                      <TableCell>{row.face_not_visible}</TableCell>
                      <TableCell>{row.multiple_faces_found}</TableCell>
                      <TableCell>{row.mobile_found}</TableCell>
                      <TableCell>{row.prohibited_object_found}</TableCell>
                    </TableRow>
                  ))}
                {emptyRows > 0 && (
